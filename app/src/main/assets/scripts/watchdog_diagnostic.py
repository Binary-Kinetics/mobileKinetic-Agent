...
def _rotate_if_needed(filepath, max_bytes=512000):
    """Rotate log file if it exceeds max_bytes. Renames to .1 and starts fresh."""
    try:
        if os.path.exists(filepath) and os.path.getsize(filepath) > max_bytes:
            rotated = str(filepath) + ".1"
            if os.path.exists(rotated):
                os.remove(rotated)
            os.rename(str(filepath), rotated)
    except OSError:
        pass  # Best effort - don't break diagnostics over rotation failure
...
class WatchdogDiagnostics:
    ...
    def _update_summary(diagnostic: dict):
        ...
        _rotate_if_needed(SUMMARY_FILE)
